...
const LoginForm = ({ locale }) => {
  ...
  return (
    <>
      ...
      <div className="flex justify-center">
        <form onSubmit={handleForm} className="w-[300px] md:w-[400px] space-y-6">
          {
            locale === "ar" ? (
              <>
                <Input
          ...
          <div className="flex flex-row mt-4">
            <Button variant="solid" color="primary" type='submit' onClick={handleForm} className="w-full">{t('login_button')}</Button>
          </div>
          <div className="flex justify-center">
            <GoogleLogin />
            <FacebookLogin />
      ...
      <Modal backdrop="opaque" isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">{message ? t('error') : t('login_success')}</ModalHeader>
              <ModalBody>
                <p>
                  {message ? message : userMessage ?? t('login_success_description')}
                </p>
              </ModalBody>
              <ModalFooter>
