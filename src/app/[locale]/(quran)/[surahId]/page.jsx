...
const surah_names = localFont({
  src: [
    {
      path: "../../../../../public/fonts/surah_names.ttf",
      weight: "400",
      style: "normal",
    },
  ],
...
const surah_text_font = localFont({
  src: [
    {
...
const Surah = ({ params: { surahId, locale } }) => {
  ...
  const [arabicSurah, setArabicSurah] = useState({});
  const [error, setError] = useState("");
  const t = useTranslations("surah");
  useEffect(() => {
    const GetArabicSurah = async () => {
      try {
        const response = await fetch(
          `http://38.242.214.31:3002/api/v1/surah/${surahId}?lang=ar`
        );
        const data = await response.json();
        if (response.ok) {
          setArabicSurah(data.surah);
        } else {
          setError(data);
        }
      } catch (error) {
    ...
    const GetReciters = async () => {
      try {
        ...
        const data = await response.json();
        setReciters(data);
        setCurrentReciter(JSON.stringify(data[0]));
      } catch (error) {
        console.error(error);
      }
    };
  ...
  const ayahsByPage = arabicSurah.ayahs?.reduce((acc, ayah) => {
  ...
  const pages = Object.keys(ayahsByPage || {});

  // Start
  /**
   * This code has been took from @asipita/number-to-arabic
   */
  var arabicNumberMap = {
    0: "٠",
    1: "١",
  ...
  function convertToArabic(number) {
    ...
    var arabicNumber = numberString
      .split("")
      .map(function (digit) {
        return arabicNumberMap[digit];
      })
      .join("");
    return arabicNumber;
  }
  ...
  return (
    <>
      <Tabs
        ...
        defaultSelectedKey={[locale === "ar" ? 0 : 1]}
      >
        <Tab key={0} title={t("reading")}>
          <div className="flex justify-center mt-6">
            <h1 className={`${surah_names.className} text-5xl`}>
              {convertToThreeDigits(surahId)}surah
            </h1>
          </div>
            ...
            <div className="mt-8">
              {pages.map((page) => (
                <section key={page} className="mt-8">
                  <div
                    className="p-4 bg-[#fff] dark:bg-[#171717] rounded w-[300px] md:w-[400px] h-auto"
                    style={{ direction: "rtl" }}
                  >
                    {ayahsByPage[page].map((ayah, index) => (
                      <span
                        key={ayah._id}
                        className={`  text-center ${
                          surah_text_font.className
                        } ${page === 1 ? `text-1xl ` : `text-3xl`}`}
                        style={{ direction: "rtl" }}
                      >
                        {" "}
                        {ayah.text}{" "}
                        <span>{convertToArabic(ayah.numberInSurah)}</span>
                      </span>
                    ))}
                  </div>
                  <div className="my-8">
                    <div className="flex justify-center">
                      <p className="text-lg text-divider mb-4">{page}</p>
                    </div>
                    <Divider />
                  </div>
                </section>
            ...
            {pages.map((page) => (
              <section key={page} className="mt-8">
                <div className="grid grid-cols-1 gap-4">
                  {ayahsByPage[page].map((ayah) => (
                    <div
                      key={ayah._id}
                      className="bg-[#fff] dark:bg-[#171717] p-4 rounded"
                      style={{ direction: "rtl" }}
                    >
                      <p className={`text-3xl ${surah_text_font.className}`}>
                        {ayah.text}{" "}
                        <span>{convertToArabic(ayah.numberInSurah)}</span>
                      </p>
